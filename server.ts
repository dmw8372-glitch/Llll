...
// API: Heartbeat to keep active room fresh
...
app.post('/api/rooms/:id/action', (req, res) => {
  ...
  if (action === 'TOGGLE_READY') {
    ...
    room.usedWords = [];
    room.turnDuration = 15.0;
  } else if (action === 'LIVE_TYPING') {
    const { liveTyping } = payload || {};
    room.liveTyping = liveTyping || null;
    broadcastToRoom(roomId, 'LIVE_TYPING', { liveTyping: room.liveTyping });
    return res.json({ success: true, liveTyping: room.liveTyping });
  } else if (action === 'SUBMIT_WORD') {
    room.liveTyping = null;
    broadcastToRoom(roomId, 'LIVE_TYPING', { liveTyping: null });

...
import { prefetchWordInDictionary } from './lib/dictionaryData';
...
export function App() {
  ...
  const normalizeRoomState = (
    ...
    return {
      ...
      liveTyping: incomingRoom.liveTyping !== undefined ? incomingRoom.liveTyping : (prevRoom?.liveTyping || null),
      createdAt: incomingRoom.createdAt || Date.now(),
    };
  };

  // Fetch real public rooms from server API and Supabase Presence
  const refreshPublicRooms = async () => {
    setIsRefreshingRooms(true);
    try {
      // 1. Request peer announcements from active hosts in lobby
      broadcastLobbyEvent('REQUEST_ROOMS', {});
      ...
      if (res.ok) {
        ...
        if (Array.isArray(data.rooms)) {
          serverRooms = data.rooms.map((r: any) => ({
            id: r.id,
            title: r.title,
      ...
      // 3. Extract active rooms from Supabase Presence
      const presenceState = lobbyChannelRef.current?.presenceState?.() || {};
      const presenceRooms: GameRoom[] = [];
      Object.values(presenceState).forEach((presences: any) => {
        if (Array.isArray(presences)) {
          presences.forEach((p: any) => {
            if (p?.room && p.room.id && p.room.status !== 'FINISHED' && p.room.isPublic !== false) {
              presenceRooms.push(p.room);
            }
          });
        }
      });
  ...
  // Supabase Global Lobby Channel for Real-time Room Sync across all players
  ...
  useEffect(() => {
    ...
    try {
      ...
      // 1. Presence Sync: Guaranteed instant room visibility for late-joining players!
      lobbyChannel.on('presence', { event: 'sync' }, () => {
        try {
          const state = lobbyChannel.presenceState();
          const discoveredRooms: GameRoom[] = [];
          Object.values(state).forEach((presences: any) => {
            if (Array.isArray(presences)) {
              presences.forEach((p: any) => {
                if (p?.room && p.room.id && p.room.status !== 'FINISHED' && p.room.isPublic !== false) {
                  discoveredRooms.push(p.room);
                }
              });
            }
          });
          ...
          if (discoveredRooms.length > 0) {
            setPublicRooms((prev) => {
              ...
              discoveredRooms.forEach((r) => map.set(r.id, r));
              return Array.from(map.values()).filter((r) => r.status !== 'FINISHED');
            });
          }
        } catch {}
      });

      // 2. Presence Leave: If a host leaves, remove their room cleanly
      lobbyChannel.on('presence', { event: 'leave' }, ({ leftPresences }: any) => {
        try {
          if (Array.isArray(leftPresences)) {
            const leftIds = new Set(leftPresences.map((p: any) => p?.room?.id).filter(Boolean));
            if (leftIds.size > 0) {
              setPublicRooms((prev) => prev.filter((r) => !leftIds.has(r.id)));
            }
          }
        } catch {}
      });

      // 3. Broadcast Events (Instant push & discovery)
      lobbyChannel
        .on('broadcast', { event: 'lobby_event' }, ({ payload }: any) => {
          ...
          } else if ((payload.type === 'ROOM_CREATED' || payload.type === 'ROOM_ANNOUNCEMENT') && payload.room) {
            setPublicRooms((prev) => {
          ...
          } else if (payload.type === 'REQUEST_ROOMS') {
            // When a new player enters the lobby, active hosts announce their room immediately!
          ...
          } else if (payload.type === 'ROOM_DELETED' && payload.roomId) {
            setPublicRooms((prev) => prev.filter((r) => r.id !== payload.roomId));
          }
        })
        .subscribe((status: string) => {
          if (status === 'SUBSCRIBED') {
            // When lobby channel connects, discover all currently active rooms immediately
            broadcastLobbyEvent('REQUEST_ROOMS', {});
            refreshPublicRooms();
          }
        });
  ...
  useEffect(() => {
    ...
    // Auto-polling (2.5s) to guarantee instant public rooms update
    const interval = setInterval(refreshPublicRooms, 2500);
    return () => {
  ...
  // Host Real-Time Room Presence & Heartbeat loop: Keeps room alive and visible to any player joining at any time
  useEffect(() => {
    if (!activeRoom || activeRoom.hostId !== myPlayerId || activeRoom.status === 'FINISHED') {
      try {
    ...
    const syncHostRoomToLobby = () => {
      const current = activeRoomRef.current || activeRoom;
      if (!current || current.status === 'FINISHED') return;

      // 1. Track in Supabase Presence so late-joining friends see the room instantly
      try {
      ...
      // 2. Send heartbeat to backend server to prevent inactivity cleanup
      const hbUrl = buildApiUrl(`/api/rooms/${encodeURIComponent(current.id)}/heartbeat`);
      fetch(hbUrl, { method: 'POST' }).catch(() => {});

      // 3. Broadcast announcement to lobby
      broadcastLobbyEvent('ROOM_ANNOUNCEMENT', { room: current });
    };
    ...
    const interval = setInterval(syncHostRoomToLobby, 3500);

    return () => {
      clearInterval(interval);
    };
  }, [activeRoom?.id, activeRoom?.hostId, activeRoom?.status, myPlayerId]);
  ...
  useEffect(() => {
    ...
    try {
      if (typeof window !== 'undefined' && 'EventSource' in window) {
        ...
        es.addEventListener('LIVE_TYPING', (e) => {
          try {
            const data = JSON.parse(e.data);
            const liveTyping = data.liveTyping || null;
            setActiveRoom((prev) => (prev ? { ...prev, liveTyping } : prev));
            if (liveTyping?.word) {
              prefetchWordInDictionary(liveTyping.word);
            }
          } catch {}
        });
  ...
  const handleCreateRoom = async (
    ...
    broadcastLobbyEvent('ROOM_ANNOUNCEMENT', { room: normalized });
    try {
      lobbyChannelRef.current?.track({
        type: 'ACTIVE_ROOM',
  ...
  // 실시간 타이핑 백그라운드 프리싱크 (타이핑 시 백그라운드로 상대방/서버에 미리 전달)
  const handleTypingWord = (payload: LiveTypingPayload | null) => {
    if (!activeRoom) return;
    setActiveRoom((prev) => (prev ? { ...prev, liveTyping: payload } : prev));
    broadcastRoomEvent('LIVE_TYPING', { liveTyping: payload });
    sendRoomAction('LIVE_TYPING', { liveTyping: payload });
  };
  ...
  const handleLeaveRoom = async () => {
    ...
    if (activeRoom) {
      ...
      if (remainingPlayers.length > 0) {
        ...
        broadcastRoomEvent('SYNC_ROOM', { room: updatedRoom });
        broadcastLobbyEvent('ROOM_ANNOUNCEMENT', { room: updatedRoom });
      } else {
        broadcastLobbyEvent('ROOM_DELETED', { roomId: activeRoom.id });
      }
    }
    setActiveRoom(null);
    setCurrentTab('HOME');

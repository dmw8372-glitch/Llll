...
import { checkWordInDictionary, prefetchWordInDictionary, getCachedWordInfo, saveWordToCache, DICTIONARY_DATABASE } from '../lib/dictionaryData';
...
export const GameView: React.FC<GameViewProps> = ({
  ...
  // Background dictionary pre-validation cache ref for 0ms instant submit
  ...
  // Realtime typing sync to background & opponents while typing
  const handleLiveTyping = (rawVal: string) => {
    ...
    // 1. Prefetch dictionary instantly on every keystroke
    prefetchWordInDictionary(cleanVal);

    // 2. Rule & Hangul & Dueum validation
    const ruleRes = validateWordRules(cleanVal, room.lastWord, room.usedWords);

    // 3. Check fast local/cached dictionary first
    ...
    if (cached && cached.isValid) {
      ...
      // Trigger background dictionary check so it's pre-resolved before user clicks submit
    ...
    // 4. Send background message to opponents & server while typing
    onTypingWord?.({
  ...
  // Unified zero-latency word submission logic (Resolves Korean IME Enter & Mobile Double-click issues)
  const processSubmit = async (wordToSubmit?: string) => {
    ...
    try {
      ...
      // Case C: Fallback to async check (already pre-fetched in background)
      if (!dictRes) {
      ...
      // 3. Clear typing state immediately
      preValidatedWordRef.current = null;
      onTypingWord?.(null);

      // 4. Success - instant 0ms submit!
      sounds.playCorrect();
      onSubmitWord(
        trimmed,
        ruleRes.isDueum ?? false,
              ...
              {room.currentPlayers.map((player) => {
                ...
                return (
                  ...
                  >
                    ...
                    {player.isAlive && (() => {
                      const isTyping = room.liveTyping && room.liveTyping.playerId === player.id && !!room.liveTyping.word;
                      const latestWord = player.wordsUsed.length > 0 ? player.wordsUsed[player.wordsUsed.length - 1] : null;

...
export function saveWordToCache(wordItem: DictionaryWord) {
...
/**
 * 0ms 즉각 캐시 확인 헬퍼
 * 타이핑 칠 때 이미 백그라운드에서 검증/캐싱된 단어가 있으면 즉시 반환
 */
export function getCachedWordInfo(word: string): { isValid: boolean; wordInfo?: DictionaryWord } | null {
  const trimmed = cleanDictWord(word);
  if (!trimmed) return null;
  if (DICTIONARY_MAP.has(trimmed)) {
    return { isValid: true, wordInfo: DICTIONARY_MAP.get(trimmed)! };
  }
  if (REAL_API_WORD_CACHE.has(trimmed)) {
    return { isValid: true, wordInfo: REAL_API_WORD_CACHE.get(trimmed)! };
  }
  return null;
}
